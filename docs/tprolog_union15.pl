...
% --- リスト値のスカラー/リスト2択と、述語シグネチャのオーバーロードの例 ---
...
% (list(K))のどちらにもなりうる場合は、単一選択肢の「真の型エイリアス」
% として KList ::= list(K). を別に用意する
% (examples/copl/EvalML4.pl/EvalML5.pl で確立したパターン)。
demo_v     ::= int_t | demo_bool.
demo_bool  ::= true | false.
demo_vlist ::= list(demo_v).

% empty/1 は「スカラーが0であること」と「リストが空であること」の
% 両方の意味で使う、意図的にオーバーロードした述語。以前の pred_sig/goal
% は最初に見つかったシグネチャに即座にコミットしてしまい、2つ目以降の
% 候補を試せなかった(match_sig がデータ構成子に対してはバックトラック
% できるのと非対称だった)。上記の pred_sig/goal の修正により、
% 同じ述語名に複数の引数リスト形式シグネチャを登録できるようになった。
empty ::= [demo_v].
empty ::= [demo_vlist].
empty(0).
empty([]).

% shrink/1 は再帰的にスカラー/リストを行き来する述語
% (examples/copl/EvalML5.pl の doesntMatch/2 と同じ構造)。
shrink ::= [demo_v].
shrink ::= [demo_vlist].
shrink([_|T]) :- shrink(T).
shrink([]).
...
test(digit_tok_is_type_checked):-
    program_clause_check_results(Results),
    memberchk(ok((digit_tok(_,_,_):-_)), Results).
...
test(pred_sig_enumerates_all_overloads):-
    findall(Ts, pred_sig(empty, Ts), All),
    sort(All, Sorted),
    Sorted == [[demo_v],[demo_vlist]].

% 型の合わない使い方は、オーバーロードを許すようになった後でも
% 依然として正しく検出されることの確認(健全性の後退が無いこと)。
% empty/1 の2つのシグネチャ([demo_v]と[demo_vlist])は、どちらも
% atom_t(アトム 'hello')を受け付けないはずなので、empty(hello) という
