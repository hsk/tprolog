...
% T2 が「述語シグネチャ([Args]/[Args]->T)」「他の型そのものへの参照(裸のatom)」
% 「組み込みコンテナ list/1 による構造的な別名」のいずれかであれば、
% 本当の型エイリアスとして扱い、カインド登録はしない。
% それ以外の複合項(例: even_succ(nat_even))は、| がなくても
% 単一構成子の直和型(newtype)とみなし、alts 経由でカインド登録する
% (そうしないと、その構成子が alts に一切登録されず、相互再帰する
%  単一構成子の型(例: nat_odd)が定義できなくなるため)。
is_true_alias_rhs(T2):- is_arg_list_form(T2), !.
is_true_alias_rhs(T2):- atom(T2), !.
...
% ここまでの ::= 宣言が出揃ったところで、まとめてカインドの整合性を検証する。
% 相互再帰的な型があっても、両方が登録済みになったこの時点でチェックすれば解決できる。
