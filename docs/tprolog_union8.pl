...
expand_bnf(T::=TE):-
    ...
    ( is_kind(T) -> check_kind_decl(T) ; true ).
...
check_kind_con(C,K):-
    (C:::ArgSorts->ResK),!,
    ( ResK==K -> true
    ; throw(error(kind_error(C,result(ResK),expected(K)),_)) ),
...
:-begin_tests(s1).
test(eval):-
    Whole = ite(true, app(lam(x,tint,var(x)+1), 41), 0),
    s1_eval(Whole,[],V), V=42.
test(type):-
    Whole = ite(true, app(lam(x,tint,var(x)+1), 41), 0),
    s1_type(Whole,[],T), T=tint.
% 否定側: -/2 は s1_expr に対して一切宣言していないので、
% s1_eval/s1_type にこの節を足すと check/2 が「弾く」ことを確認する。
...
test(list_eval):-
    L = econs(1,econs(2,econs(3,enil))),
    s1_eval(L,[],V),
...
test(kind_decl_time_check_rejects_bad_arg):-
    kind(demo_bad_ty),
        ...
        ( expand_bnf(demo_bad_ty::=(demo_ok|demo_mk(demo_bad_ty,s1_expr))),
