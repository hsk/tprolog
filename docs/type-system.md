...
## 既知の限界: 部分型判定(⊢/<:)の緩さによる誤判定

`tprolog.pl` の部分型判定エンジンには、意図的に直さないことにした(直すと既存の多くのファイルが壊れるため)既知の緩さが複数ある。

1. **単一要素の構成子名リスト同士が無関係でも部分型になる**。`Γ⊢T1<:T2:-is_list(T1),is_list(T2),!,forall(member(X,T1),([T1<:T2|Γ]⊢[X]<:T2)).` という節が、要素 `X` を `[X]` という単集合リストに包んで再帰呼び出しするため、コインダクション用のメモ化(`Γ` へのメンバーシップチェック)が早期に真を返してしまい、`X` の中身を検証しないまま常に成功する。カインドの構成子名リストが1個(または少数)で、複数のカインドが同じ構成子名を共有していると特に発現する(`examples/copl/EvalML4.pl` の `arm_nil`/`arm_cons` 調査で発見)。
2. **同じ裸アトムを複数カインドが共有すると、宣言順で「代表」が固定される**。`tp(_,M,T):-atom(M),(M:::[]->T1),!,[]⊢T1<:T.` が、アトム `M` に対して最初に見つかった構成子事実にだけコミットしてしまうため、`nil` のような値が複数のカインド(式ASTとパターンASTなど)で共有されていると、片方専用の値がもう片方としても誤って型検査を通ってしまうことがある(`examples/copl/EvalML5.pl`/`TypingML4.pl` で発見)。
3. **自分自身がカインド登録されたアトムは、他の複数選択肢カインドほぼ何にでも緩く適合する**。継続渡し形式(CPS)の評価器(`examples/copl/EvalContML1.pl`)で、継続の終端マーカー `'_'`(`cont ::= '_' | ...` の一部として登録される)が、`cont` と無関係な `v`/`e` のような複数選択肢カインドに対してもことごとく部分型判定を通ってしまうことを発見した(単一のプリミティブカインドに対しては正しく失敗する)。原因は上記1・2とは別経路の緩さで、完全には特定できていない。この影響で「値の一部が未評価であることを示すプレースホルダ(穴)」を位置ごとに型で区別する設計は機能しない。

これらはいずれも「型の合わないプログラムを誤って受理してしまう」方向の不健全さであり、逆(正しいプログラムを誤って拒否する)は起きにくい。
実用上は、同じアトム/構成子名を複数の意味論的カテゴリで再利用する設計を避けるか、それができない場合は制限としてコメントで明記し、実際に使われている節・テストが正しく通ることだけを確認する運用にしている。
厳密に塞ぐには `tprolog.pl` の部分型判定アルゴリズム自体を見直す必要があり、今後の拡張課題として残っている。
